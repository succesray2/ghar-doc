...
export const TRIAGE_RULE_VERSION = 2;
...
export const BODY_REGION_OPTIONS: { id: string; label: string }[] = [
...
const ABDOMINAL_PAIN_SIGNS: AssociatedSignQuestion[] = [
...
const DIABETES_SIGNS: AssociatedSignQuestion[] = [
  { id: 'confusion', label: 'Confusion?' },
  { id: 'drowsiness', label: 'Drowsiness?' },
  { id: 'unresponsive', label: 'Difficult to wake / unresponsive?' },
];

const BP_SIGNS: AssociatedSignQuestion[] = [
...
const FEVER_SIGNS: AssociatedSignQuestion[] = [
...
export const CHEST_PAIN_IDS = ['chest_pain', 'chest_pressure', 'chest_tightness', 'chest_discomfort', 'chest_heaviness'];
export const FALL_INJURY_IDS = ['fall', 'recent_fall', 'head_injury', 'road_traffic_accident', 'sports_injury'];
export const GI_IDS = ['vomiting', 'diarrhea', 'repeated_vomiting'];
export const ABDOMINAL_PAIN_IDS = ['abdominal_pain', 'upper_abdominal_pain', 'lower_abdominal_pain', 'right_abdominal_pain', 'left_abdominal_pain'];
export const DIABETES_CATEGORY_IDS = [
...
export const DIABETES_EXTREME_IDS = ['very_high_blood_sugar', 'very_low_blood_sugar'];
export const BP_EXTREME_IDS = ['very_high_bp_reading', 'low_bp_reading'];
export const FEVER_IDS = ['infection_fever', 'fever'];
export const ELDERLY_IDS = [
  'recent_fall',
  'new_confusion',
...
export const WOMENS_HEALTH_BLEEDING_IDS = ['abnormal_vaginal_bleeding', 'heavy_bleeding'];
...
export const ASSOCIATED_SIGN_QUESTIONS: Record<string, AssociatedSignQuestion[]> = {
  ...Object.fromEntries(CHEST_PAIN_IDS.map((id) => [id, CHEST_PAIN_SIGNS])),
  ...Object.fromEntries(BREATHING_IDS.map((id) => [id, BREATHING_DIFFICULTY_SIGNS])),
  ...Object.fromEntries(NEURO_SYMPTOM_IDS.map((id) => [id, NEUROLOGICAL_SIGNS])),
  ...Object.fromEntries(GI_IDS.map((id) => [id, GI_SIGNS])),
  ...Object.fromEntries(ABDOMINAL_PAIN_IDS.map((id) => [id, ABDOMINAL_PAIN_SIGNS])),
  ...Object.fromEntries(FALL_INJURY_IDS.map((id) => [id, FALL_INJURY_SIGNS])),
  ...Object.fromEntries(DIABETES_EXTREME_IDS.map((id) => [id, DIABETES_SIGNS])),
  ...Object.fromEntries(BP_EXTREME_IDS.map((id) => [id, BP_SIGNS])),
  ...Object.fromEntries(FEVER_IDS.map((id) => [id, FEVER_SIGNS])),
  ...Object.fromEntries(ELDERLY_IDS.map((id) => [id, ELDERLY_SIGNS])),
  ...Object.fromEntries(WOMENS_HEALTH_BLEEDING_IDS.map((id) => [id, WOMENS_HEALTH_BLEEDING_SIGNS])),
};
...
function isSevere(symptom: SymptomAnswer | undefined): boolean {
  return symptom?.severity === SeverityOption.SEVERE || symptom?.severity === SeverityOption.VERY_SEVERE;
}
function anySelected(answers: TriageAnswers, ids: string[]): boolean {
...
const RED_FLAG_RULES: TriageRule[] = [
  ...
  },
  {
    ...
    priority: 'RED',
    test: (a) =>
      !!findSymptom(a, 'difficulty_speaking') ||
  ...
  },
  {
    ...
    test: (a) => {
      ...
      return (
        ...
        anySign(a, FEVER_IDS, 'reduced_consciousness') ||
        anySign(a, FEVER_IDS, 'confusion')
      );
    },
  },
  {
    ...
    test: (a) =>
      ...
      anySign(a, WOMENS_HEALTH_BLEEDING_IDS, 'soaking_through_protection') ||
      anySign(a, WOMENS_HEALTH_BLEEDING_IDS, 'fainting'),
  },
  {
    ...
    test: (a) => {
      ...
      if (!h) return false;
      return (
        !!findSymptom(a, 'sudden_severe_headache') ||
  ...
  },
  {
    ...
    priority: 'RED',
    test: (a) => {
      const extreme = findAnySymptom(a, DIABETES_EXTREME_IDS);
      return !!extreme && (anySign(a, DIABETES_EXTREME_IDS, 'confusion') || anySign(a, DIABETES_EXTREME_IDS, 'drowsiness') || anySign(a, DIABETES_EXTREME_IDS, 'unresponsive'));
    },
  },
  {
    ...
    priority: 'RED',
    test: (a) => {
      const extreme = findAnySymptom(a, BP_EXTREME_IDS);
      return (
        !!extreme &&
        (anySign(a, BP_EXTREME_IDS, 'severe_headache') ||
          anySign(a, BP_EXTREME_IDS, 'chest_pain') ||
          anySign(a, BP_EXTREME_IDS, 'confusion') ||
          anySign(a, BP_EXTREME_IDS, 'vision_change'))
      );
    },
  },
  {
    ...
    priority: 'RED',
    test: (a) => anySign(a, FEVER_IDS, 'reduced_consciousness') || anySign(a, FEVER_IDS, 'confusion'),
  },
];
...
const ORANGE_FLAG_RULES: TriageRule[] = [
  {
    ...
    priority: 'ORANGE',
    test: (a) => anySelected(a, ELDERLY_IDS) || anySign(a, ELDERLY_IDS, 'significantly_different_from_usual'),
  },
  {
  ...
  },
  {
    ...
    priority: 'ORANGE',
    test: (a) => anySelected(a, DIABETES_EXTREME_IDS),
  },
  {
    id: 'bp_extreme_reading',
    label: 'Extreme blood pressure reading reported',
    priority: 'ORANGE',
    test: (a) => anySelected(a, BP_EXTREME_IDS),
  },
  {
    id: 'high_fever',
    label: 'Fever reported',
    priority: 'ORANGE',
    test: (a) => isSevere(findAnySymptom(a, FEVER_IDS)),
  },
  {
    id: 'womens_health_bleeding',
    label: 'Abnormal or heavy bleeding reported',
    priority: 'ORANGE',
    test: (a) => anySelected(a, WOMENS_HEALTH_BLEEDING_IDS),
  },
  {
    ...
    test: (a) => a.symptoms.some((s) => s.severity === SeverityOption.SEVERE || s.severity === SeverityOption.VERY_SEVERE),
  },
];
