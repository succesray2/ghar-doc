  ...
  return (
    <View>
        ...
        {filteredCategories.map((cat) => {
          ...
          return (
            <View key={cat.id} style={styles.categoryBlock}>
              <Pressable style={styles.categoryHeader} onPress={() => setOpenCategoryId(isOpen ? null : cat.id)}>
              ...
              {isOpen && (
                <View style={styles.symptomList}>
                  {cat.symptoms.map((s) => {
                    ...
                    return (
                      <Pressable key={s.id} style={styles.symptomRow} onPress={() => onToggleSymptom(s.id)}>
                        <Feather
      ...
      <TextInput
        ...
        onChangeText={(v) => onChange({ otherSymptomText: v })}
...
const styles = StyleSheet.create({
  ...
  categoryHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 12 },
  categoryTitle: { fontFamily: fonts.semiBold, fontSize: 14, color: colors.text },
  categoryCount: { fontFamily: fonts.regular, fontSize: 12, color: colors.teal600 },
  symptomList: { paddingHorizontal: 14, paddingBottom: 8 },
  symptomRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 7 },
  symptomLabel: { fontFamily: fonts.regular, fontSize: 14, color: colors.text },
