...
import type { AdminCreateNurseInput, NurseListItemDto, NurseStatus } from '@ghar-doc/shared';
...
export function useNurses(status?: NurseStatus) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<NurseListItemDto[]>('/nurses', { params: status ? { status } : undefined });
      return data;
    },
  });
}
