...
import type { AdminCreatePhysiotherapistInput, PhysiotherapistListItemDto, PhysiotherapistStatus } from '@ghar-doc/shared';
...
export function usePhysiotherapists(status?: PhysiotherapistStatus) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<PhysiotherapistListItemDto[]>('/physiotherapists', { params: status ? { status } : undefined });
      return data;
    },
  });
}
