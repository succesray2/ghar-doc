...
// markdown convention as apps/mobile/src/screens/StaticInfoScreen.tsx
// (# / ## / ### headers, "* " bullets, whole-line **bold**, "| a | b |"
// tables). Not a general markdown renderer, just enough structure for a
// long legal document to actually scan on a page.
export function LegalDocument({ body }: { body: string }) {
  return <div className="mx-auto max-w-3xl px-4 py-16 md:py-24">{renderBody(body)}</div>;
}
...
function renderBody(body: string) {
  ...
  const flushTable = (key: string) => {
    ...
    blocks.push(
      <div key={key} className="my-4 overflow-x-auto rounded-xl border border-line">
        <table className="w-full text-left text-sm">
          <thead className="bg-bg-soft">
            <tr>
              {header.map((cell, i) => (
                <th key={i} className="px-4 py-2.5 font-semibold text-navy-900">
                  {cell}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-t border-line">
                {row.map((cell, j) => (
                  <td key={j} className="px-4 py-2.5 text-ink-600">
                    {cell}
                  </td>
                ))}
              </tr>
