// Real content, kept identical to apps/mobile/src/data/privacy.ts and
// apps/marketing/src/data/privacy.ts on purpose (same policy should read
// the same everywhere) — authored directly against the real GHARDoc
// codebase, not a template. Where a section doesn't apply yet (no payment
// processor, no diagnostics, no analytics, no self-serve deletion), it
// says so plainly instead of describing a feature that doesn't exist.
// Bracketed placeholders ([DD Month YYYY], [registered address], etc.) are
// unfilled legal/business details, not invented values.
//
// This document has NOT been reviewed by a lawyer and must be before
// production launch with real patients.
