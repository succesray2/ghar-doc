...
export function AssignedVisitsPage() {
  ...
  return (
    <div className="space-y-4">
      {visits.map((visit) => {
        const nextTransitions = getLegalTransitions(visit.status, Role.PHYSIOTHERAPIST).filter((t) => t.to !== 'CANCELLED');
        return (
          <Card key={visit.id}>
            ...
            {visit.serviceDetails && 'conditionType' in visit.serviceDetails && (
              <p className="text-sm text-slate-600">
                Condition: {visit.serviceDetails.conditionType} · Mobility: {visit.serviceDetails.mobilityLevel}
                {visit.serviceDetails.sessionGoal ? ` — ${visit.serviceDetails.sessionGoal}` : ''}
              </p>
            )}
