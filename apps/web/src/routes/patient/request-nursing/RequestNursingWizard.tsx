...
import { NursingDetailsStep } from './NursingDetailsStep';
import { LocationReviewStep } from './LocationReviewStep';
import { INITIAL_NURSING_WIZARD_STATE, type NursingWizardState } from './types';
...
export function RequestNursingWizard() {
  ...
  const handleSubmit = () => {
    ...
    const reasonForVisit =
      state.nursingServiceType === 'OTHER' ? state.otherServiceText : `Nursing: ${state.nursingServiceType}`;
      ...
      <div className="mt-6 flex justify-between">
        ...
        {step < TOTAL_STEPS ? (
          ...
          <Button onClick={handleSubmit} disabled={createVisit.isPending}>
            {createVisit.isPending ? 'Submitting…' : 'Request nursing care'}
          </Button>
        )}
      </div>
    </Card>
